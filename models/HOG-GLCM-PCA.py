...
def GET_HOG_GLCM(img_dir, dim=(64,128),
            bins=9, 
            pixels_per_cell=(8,8),
            cells_per_block=(2,2),
            norm='L2',
    ...
    Args:
        ...
        dim : image dimension
    """
    write_txt =('D:\\ClarentiCelsia\\Personal\\SKRIPSI\\Proposal\\HOG_GLCM[%s][%s].txt' %(distance, angle))
    ...
    with open(write_txt, "a")as f:
        ...
        for folder in os.listdir(img_dir):
            ...
            for file in os.listdir(img_file):
                ...
                glcm_features, fd = HOGLCM_feature(imgs, dim, bins, pixels_per_cell, cells_per_block, norm, distance, angle)
                
                label_name = img_file.split("/")[-1].split("_")[0] #leaf10
                label_num = img_label[label_name] 
             
                row = image_path + "\t" + str(label_num) + "\t" + str(glcm_features).split("[[")[-1].split("]]")[0] + " " #path/xx.tif 1
...
def HOGLCM_feature(imgs, 
    ...
    #Texture 
    grays = skimage.img_as_ubyte(img) 
    grays = np.asarray(grays, dtype="int32") 
    
    glcm = greycomatrix(grays, [1], [0], levels=grays.max()+1, symmetric=True, normed=True)
    ...
    if not contrast is None or not energy is None or not homogeneity is None or  not correlation is None:
        params = [contrast, energy, homogeneity, correlation]
...
def load_txt(hogtxt):
    ...
    with open(hogtxt, "r")as f:
        ...
        for row_data in data:
            ...
            hoglcm_features = hog_str.split(" ")
            hoglcm_features = [float(hog) for hog in hoglcm_features]
            #print "hog feature length = ", len(hog_feature)
            image_names.append(img_name)
            labels.append(int(label))
            features.append(hoglcm_features)
...
def Naive(distance=1, angle=0, visualize=False):
    ...
    print('Accuracy of GNB classifier on testing set: {:.5f}'
    ...
    # Accuracy of prediction
    print("Accuracy GNB: " + str(accuracy_score(y_pred, y_test)))
    ...
    # using cross val
    test_scores = cross_val_score(gnb, X_test, y_test, scoring='accuracy', cv=7)
    print("Accuracy of testing set by using cross val : ",test_scores)
    print("Mean of test scores: ", test_scores.mean())
    
    train_scores = cross_val_score(gnb, X_train, y_train, scoring='accuracy', cv=7)
    print("Accuracy of training set by using cross val : ",train_scores)
    print("Mean of test scores: ", train_scores.mean(), "\n")
