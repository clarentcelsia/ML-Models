# -*- coding: utf-8 -*-
"""
Created on Sat Sep 19 23:10:36 2020
...
def Naive(d, angle, visualize=False):
    ...
    print('GNB Accuracy (training): ', train_accuracy)
    print('GNB Accuracy (testing): ', test_accuracy)
    ...
    print(' Accuracy of GNB Prediction: {:.3f}'.format(pred_accuracy))
        
    # through confusion matrix
      # accuracy, precision, recall, f-score
    confusionmatrix = confusion_matrix(y_test, y_pred)
    classificationreport = classification_report(y_test, y_pred)
    print(' Accuracy of GNB Prediction using ConfusionMTX:\n', confusionmatrix, "\n")
    ...
    train_scores = cross_val_score(gnb, X_train, y_train, scoring='accuracy', cv=5)
    ...
    if visualize:
        ...
        plt.title(" GNB-GLCM[%s][%s]" % (d, angle))
        plt.xlabel('X1')
        plt.ylabel('X2')
        plt.legend()
        plt.grid()
...
if __name__=="__main__":
    ...
    Naive(d=1, angle=0, visualize=True)
